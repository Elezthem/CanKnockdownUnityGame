...
public class LevelManager : MonoBehaviour
{
    ...
    public void SpawnBall()
    {
        if(availableBalls>0)
        {
           ...
           SoundManager.instance.PlayFx(FxTypes.BALLSPAWN);      
        }
        else
        {
            if(GameManager.singleton.gameStatus ==GameStatus.Playing)
                ...
                UIManager.instance.GameResult(GameStatus.Failed);
    ...
    {
        LevelData level=Instantiate(levelDatas[levelIndex] , new Vector3( 0 , 2.5f , 8.5f) , Quaternion.identity).GetComponent<LevelData>();
    ...
    public void ReduceCan()
    {
        ...
        if(GameManager.singleton.gameStatus ==GameStatus.Playing)
        {
            if(availableCans<=0)
            {
                ...
                UIManager.instance.GameResult(GameStatus.Complete);
            }
        }
    }
